...
# MongoDB connection with proper configuration for Atlas
...
async def get_status_checks(skip: int = 0, limit: int = 100):
    # Validate and cap limit to prevent excessive data retrieval
    limit = min(max(limit, 1), 100)  # Between 1 and 100
    skip = max(skip, 0)  # No negative skip values
...
app.add_middleware(
    ...
    allow_origins=os.environ.get('CORS_ORIGINS', '*').strip('"').split(','),
    allow_methods=["*"],
    allow_headers=["*"],
)
